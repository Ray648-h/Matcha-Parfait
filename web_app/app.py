...
try:
    ...
    hpoi_service_module = try_import('hpoi_search.hpoi_service')
    if hpoi_service_module:
        from hpoi_search.hpoi_service import search_hpoi_top_5
        print("成功导入HPOI服务")
    else:
    ...
    # 尝试导入闲鱼搜索
    try:
        xianyu_module = try_import('service.search.xianyu_direct_search')
        if xianyu_module:
            from service.search.xianyu_direct_search import search_xianyu
