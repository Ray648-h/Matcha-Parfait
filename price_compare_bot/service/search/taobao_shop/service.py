...
REQUEST_RETRY = 6  # 增加重试次数
REQUEST_TIMEOUT_MS = 45000
SHORT_DELAY_RANGE_MS = (2000, 5000)  # 增加短等待时间
LONG_DELAY_RANGE_MS = (8000, 15000)  # 增加长等待时间
...
def _is_anti_bot_page(page) -> bool:
    try:
        ...
        anti_bot_title_keywords = [
            "验证", "验证码", "异常流量", "访问受限", "安全验证",
            "安全中心", "人机验证", "风险验证", "安全检测",
            "verify", "security", "captcha", "robot check"
        ]
        if any(keyword in title for keyword in anti_bot_title_keywords):
            return True
    except Exception:
        pass
    ...
    try:
        ...
        anti_bot_markers = [
            "验证码", "滑动", "异常流量", "访问受限", "captcha",
            "请完成验证", "拖动滑块", "安全验证", "人机验证",
            "risk control", "security check", "robot detection",
            "请输入验证码", "点击验证", "智能验证",
            "geetest", "tencent captcha", "aliyun captcha",
            "您的访问过于频繁", "访问频率过高", "请稍后再试"
        ]
    ...
    try:
        # 检查常见的验证码元素
        captcha_selectors = [
            ".geetest_holder", ".captcha-container", ".verify-box",
            "#captcha", ".slider-container", ".nc-container"
        ]
...
def _safe_goto(page, url: str, debug: bool = False, retry: int = REQUEST_RETRY) -> bool:
    for attempt in range(1, retry + 1):
        ...
        try:
            ...
            if page.is_closed():
                if debug:
                    print(f"[_safe_goto] 页面被关闭（可能是反爬机制），第{attempt}次: {url}")
                return False
